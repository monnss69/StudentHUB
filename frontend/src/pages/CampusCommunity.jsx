...
const CampusCommunity = () => {
  // Filter posts for Campus Community category
  const campusPosts = mockData.posts.filter(
    post => post.category === "Campus Community"
  );
          ...
          >
            ...
            <div className="flex flex-wrap gap-2">
              {post.tags.map((tag, index) => (
                <span 
                  key={index}
                  className="px-3 py-1 bg-green-100 text-green-600 rounded-full text-sm" // Changed color to green
                >
                  {tag}
                </span>
              ))}
